...
def download_ftp_tree(ftp_handle, path, destination, pattern=None, overwrite=False, guess_by_extension=True):
    ...
    os.chdir(os.path.splitdrive(destination)[0]) #(destination) # change working directory to ftp mirror directory
...
if __name__ == "__main__":
    ...
    ftp = ftplib.FTP(mysite, username, password)
    ftp.cwd('Temp')
    filename='2021_11_29(3).zip'
    #ftp.retrbinary('RETR ' +filename , open('D:/'+filename,'wb').write)
    ftp.storbinary( 'STOR '+filename, open(os.path.join('D:/',filename), 'rb') )
